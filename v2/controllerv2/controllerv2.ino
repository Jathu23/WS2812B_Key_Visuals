...
// Key reactive style
String keyReactiveStyle = "center_ripple";
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (command.startsWith("KEY")) {
      ...
      if (commaIdx > 0) {
        keyReactiveStyle = command.substring(commaIdx + 1);
      } else {
        keyReactiveStyle = "center_ripple";
      }
      triggerKeyReactiveEffect();
    }
...
void triggerKeyReactiveEffect() {
  if (keyReactiveStyle == "center_ripple") {
    createCenterRippleEffect();
  } else if (keyReactiveStyle == "pulse") {
    createCenterPulseEffect();
  } else if (keyReactiveStyle == "color_burst") {
    createCenterColorBurstEffect();
  } else {
    createCenterRippleEffect(); // fallback
  }
}
...
// New: Center Pulse Effect
void createCenterPulseEffect() {
  ...
  for (int r = 0; r < NUM_LEDS / 2; r++) {
    int left = center - r;
    int right = center + r;
    if (left >= 0) leds[left] = CHSV(160, 255, 255 - r * 8);
    if (right < NUM_LEDS) leds[right] = CHSV(160, 255, 255 - r * 8);
    FastLED.show();
    delay(10);
  }
}

// New: Center Color Burst Effect
void createCenterColorBurstEffect() {
  // Burst rainbow from center
  for (int i = 0; i < NUM_LEDS; i++) {
    leds[i] = CRGB::Black;
  }
  int center = NUM_LEDS / 2;
  for (int r = 0; r < NUM_LEDS / 2; r++) {
    int left = center - r;
    int right = center + r;
    uint8_t hue = map(r, 0, NUM_LEDS / 2, 0, 255);
